...
const MemberList = ({ tab, privateChats, setTab, userData }) => {
  ...
  return (
            <div className="member-list cursor-default text-[#8d8f92] flex justify-center">
                <ul>
                    <li onClick={()=>{setTab("CHATROOM")}} className= 'max-w-[100%] break-words cursor-pointer mt-6  hover:bg-primary hover:rounded-md hover:text-[#ffffffb9] font-thin md:text-lg text-sm'>PUBLIC MESSAGES</li>
                    ...
                    <li onClick={()=>{ {setTab(name)}}} className='py-1 break-all mr-1 ml-1 cursor-pointer hover:bg-primary hover:rounded-md hover:text-[#ffffffb9]' key={index}> {name === userData.username ? '' : name}  </li> ) : null
                    ))}
                </ul>
