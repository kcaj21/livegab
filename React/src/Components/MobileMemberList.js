    ...
    <div className="member-list cursor-default text-[#8d8f92] flex justify-center">
      <ul className="">
        <li
          ...
          className={`max-w-[100%] text-center break-words cursor-pointer ml-2 mt-6 hover:bg-primary hover:rounded-md hover:text-[#ffffffb9] font-thin text-lg ${ tab === 'CHATROOM' ? 'text-[#7289da] bg-primary rounded-md ' : ''}`}
        >
          PUBLIC MESSAGES
        </li>
        ...
        <div className="overflow-y-auto max-h-[80vh]">
          {privateChatUsers.map((name, index) => (
            <li
              ...
              className={`py-1 px-1 break-all mr-1 ml-1 cursor-pointer hover:bg-primary hover:rounded-md hover:text-[#ffffffb9] text-xl ${ name === tab ? 'text-[#ffffffb9] bg-primary rounded-md' : ''}`}
              key={index}
            >
